...
foreach ($f in $files) {
  ...
  $hasMojibake = ($text -match 'â•|â€“|â€”|â€"|â€™|â€œ|â€\x9d')
...
if (Test-Path "eve_server.py") {
    $b = [System.IO.File]::ReadAllBytes("eve_server.py")
    $healthy = 0
    for ($i=0; $i -lt $b.Length - 2; $i++) {
        if ($b[$i] -eq 0xE2 -and $b[$i+1] -eq 0x95 -and $b[$i+2] -eq 0x90) { $healthy++ }
    }
    $corrupt = 0
    for ($i=0; $i -lt $b.Length - 4; $i++) {
        if ($b[$i] -eq 0xC3 -and $b[$i+1] -eq 0xA2 -and $b[$i+2] -eq 0xE2 -and $b[$i+3] -eq 0x80 -and $b[$i+4] -eq 0xA2) { $corrupt++ }
    }
    Write-Host "VERIFY_eve_server.py: healthy_box_chars=$healthy corrupt_mojibake=$corrupt"
}
